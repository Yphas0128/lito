package com.oltocoder.boot.framework.quartz.core.scheduler;

import com.oltocoder.boot.framework.quartz.core.enums.JobDataKeyEnum;
import com.oltocoder.boot.framework.quartz.core.handler.JobHandlerInvoker;
import org.quartz.*;
...
public class SchedulerManager {
    ...
    // ==
    public void addJob(Long jobId,String jobName, String jobHandlerName, String jobHandlerParam, String cronExpression,
                       Integer retryCount, Integer retryInterval)
            throws SchedulerException {
        JobDetail jobDetail = JobBuilder.newJob(JobHandlerInvoker.class)
                .usingJobData(JobDataKeyEnum.JOB_ID.name(), jobId)
    ...
    public void triggerJob(Long jobId, String jonName,String jobHandlerName, String jobHandlerParam)
            throws SchedulerException {
        JobDataMap data = new JobDataMap(); // 无需重试，所以不设置 retryCount 和 retryInterval
        data.put(JobDataKeyEnum.JOB_ID.name(), jobId);
        data.put(JobDataKeyEnum.JOB_HANDLER_NAME.name(), jobHandlerName);
        data.put(JobDataKeyEnum.JOB_HANDLER_PARAM.name(), jobHandlerParam);
        // 触发任务
        scheduler.triggerJob(new JobKey(jonName), data);
    }
}
